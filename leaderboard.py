...
def upload_to_google_sheets(leaderboard: List[Dict]):
    """Google Sheets에 리더보드 업로드"""
    if not GOOGLE_SHEETS_ENABLED:
        logger.info("Google Sheets 연동이 비활성화되어 있습니다.")
        return

    if not GOOGLE_SHEET_ID:
        logger.warning("GOOGLE_SHEET_ID가 설정되지 않았습니다. Google Sheets 업로드를 건너뜁니다.")
        return

    try:
        logger.info("Google Sheets에 업로드 중...")

        # 서비스 계정 인증
        SCOPES = ['https://www.googleapis.com/auth/spreadsheets']

        if not os.path.exists(GOOGLE_SHEETS_CREDENTIALS_FILE):
            logger.error(f"인증 파일을 찾을 수 없습니다: {GOOGLE_SHEETS_CREDENTIALS_FILE}")
            return
        ...
        try:
            ...
            worksheet = spreadsheet.add_worksheet(title='Leaderboard', rows=100, cols=10)

        # 데이터 준비
        headers = ['순위', '참여자', '채널', '뱃지', '최종', '기본', '참여', '바이럴', '성장', '영상 수']
        data = [headers]
        ...
        # 1-3위 강조
        for i, item in enumerate(leaderboard[:3], 2):
            if item['status'] == 'success':
                if i == 2:  # 1위
                    color = {'red': 1.0, 'green': 0.95, 'blue': 0.7}  # 금색
                elif i == 3:  # 2위
                    color = {'red': 0.85, 'green': 0.85, 'blue': 0.85}  # 은색
                elif i == 4:  # 3위
                    color = {'red': 1.0, 'green': 0.8, 'blue': 0.6}  # 동색

                worksheet.format(f'A{i}:J{i}', {'backgroundColor': color})
        ...
        worksheet.update(f'A{len(data) + 2}', [[f'마지막 업데이트: {update_time}']])

        logger.info(f"Google Sheets 업로드 완료: https://docs.google.com/spreadsheets/d/{GOOGLE_SHEET_ID}")

    except Exception as e:
        logger.error(f"Google Sheets 업로드 실패: {e}")
        logger.error(f"에러 상세: {type(e).__name__}")
...
def main():
    ...
    # Google Sheets 업로드
    upload_to_google_sheets(leaderboard)
